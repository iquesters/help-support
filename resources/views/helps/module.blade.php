            ...
            <div class="col-lg-4 col-md-6">
                <div class="border rounded-3 p-3 h-100 d-flex flex-column">
                    <div class="d-flex align-items-center gap-2 mb-2">
                        <i class="{{ $mod->getMeta('module_icon') ?: 'fa-solid fa-cube' }}"></i>
                        <h6 class="fw-bold small mb-0 text-body">{{ $mod->name }}</h6>
                    </div>
                    <p class="text-secondary small mb-3 flex-grow-1">{{ $mod->description }}</p>
                    <div>
                        <a href="{{ route('helpsupport.ui.show', ['viewName' => 'helps.docs']) }}?module={{ urlencode($mod->name) }}&uid={{ urlencode($mod->uid) }}" class="btn btn-sm btn-outline-dark">
